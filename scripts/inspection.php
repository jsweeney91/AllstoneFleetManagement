...
<div class="container">
	...
	<?php
		...
		if(isset($_GET["id"])){
			$sql = "select v.registration,v.in_service, i.inspection_date,i.comments from vehicle b, inspection i where i.vehicle = v.code and i.code=".$_GET["id"];
			
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {	
					$continue = true;				
					$regno=$row["regisration"];
					if(row["in_service"]=="1"){
